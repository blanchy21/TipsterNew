...
const UserManagement: React.FC<UserManagementProps> = ({ onUserSelect }) => {
  ...
  useEffect(() => {
    ...
    switch (filterType) {
      case 'new':
        ...
        filtered = filtered.filter((user) => new Date(user.memberSince || '') > oneWeekAgo);
        break;
      case 'verified':
  ...
  const getNewUserCount = () => {
    const oneWeekAgo = new Date();
    oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);
    return users.filter((user) => new Date(user.memberSince || '') > oneWeekAgo).length;
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-semibold text-white">
                {users.length - getVerifiedUserCount()}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex gap-2">
            {[
              ...
              { key: 'verified', label: 'Verified', count: getVerifiedUserCount() },
              {
                key: 'unverified',
                label: 'Unverified',
                count: users.length - getVerifiedUserCount(),
              },
            ].map((filter) => (
              <button
                key={filter.key}
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {user.isVerified && <Shield className="w-4 h-4 text-primary" />}
                    {isNewUser(user) && (
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-surface-2 rounded-2xl p-6 max-w-md w-full">
            ...
            <p className="text-zinc-300 mb-6">
              ...
              <strong className="text-white">{deleteConfirmUser.name}</strong>? This action will
              permanently remove the user and all their associated data including:
            </p>
