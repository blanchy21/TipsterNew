...
export const getMobileFirestoreFunctions = async () => {
  ...
  try {
    ...
    if (DEBUG_MOBILE_FIREBASE)
      console.error(
        `[Mobile Debug] Firestore functions load failed after ${loadTime.toFixed(2)}ms:`,
        error
      );
    return null;
  }
...
export const getMobileAuthFunctions = async () => {
  ...
  try {
    ...
    if (DEBUG_MOBILE_FIREBASE)
      console.error(
        `[Mobile Debug] Auth functions load failed after ${loadTime.toFixed(2)}ms:`,
        error
      );
    return null;
  }
