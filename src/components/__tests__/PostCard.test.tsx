  ...
  return function MockLikeButton({ post, onLikeChange }: any) {
    return (
      <button
        ...
        onClick={() =>
          onLikeChange && onLikeChange(post.id, post.likes + 1, [...post.likedBy, 'user1'])
        }
      >
        Like ({post.likes})
      </button>
...
describe('PostCard', () => {
  ...
  it('calls onLikeChange when like button is clicked', () => {
    ...
    expect(defaultProps.onLikeChange).toHaveBeenCalledWith(mockPost.id, mockPost.likes + 1, [
      ...mockPost.likedBy,
      'user1',
    ]);
  });
  ...
  it('formats date correctly', () => {
    ...
    // Check if a relative date is displayed (e.g., "1y", "2y", etc.)
    // The exact value depends on the current date relative to 2024-01-01
    const dateElement = screen.getByText(/^\d+y$/);
  ...
  it('handles long content with truncation', () => {
    const longContent =
      'This is a very long post content that should be truncated when displayed in the feed to maintain a clean layout and prevent the post from taking up too much vertical space on the screen.';
