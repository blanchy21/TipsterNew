      ...
      <div className="flex items-center justify-between p-4 border-b border-white/10 bg-surface-2/50 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-400">{isLoading ? 'Analyzing...' : 'Ready to help'}</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-thin">
          {QUICK_ACTIONS.map((action) => (
            <button
              ...
              disabled={isLoading}
              className="px-3 py-1.5 bg-white/10 hover:bg-white/20 rounded-full text-xs text-zinc-300 whitespace-nowrap transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {action.label}
            </button>
          ))}
