...
const SimpleAdminPage: React.FC = () => {
  ...
  const handlePopulateData = async () => {
    ...
    try {
      console.warn('Starting to populate test data...');
      const result = await populateTestData();
      console.warn('Populate result:', result);

      if (result.success) {
        setMessage({
          type: 'success',
          text: 'Test data populated successfully! Refresh the page to see the changes.',
        });
      } else {
        setMessage({
          type: 'error',
          text: 'Failed to populate test data. Check console for details.',
        });
      }
    } catch (error) {
  ...
  const handleClearData = async () => {
    ...
    try {
      console.warn('Starting to clear test data...');
      const result = await clearTestData();
      console.warn('Clear result:', result);

      if (result.success) {
        setMessage({
          type: 'success',
          text: 'Test data cleared successfully! Refresh the page to see the changes.',
        });
      } else {
        setMessage({
          type: 'error',
          text: 'Failed to clear test data. Check console for details.',
        });
      }
    } catch (error) {
          ...
          <div className="space-y-6">
            <div className="bg-white/5 rounded-xl p-6 border border-white/10">
              ...
              <p className="text-slate-300 mb-4">
                This is a simplified admin dashboard for testing purposes. You can use the Data
                Management tab to populate or clear test data.
              </p>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white/5 rounded-xl p-6 border border-white/10">
                ...
                <p className="text-neutral-400 mb-6">
                  Add sample users, posts, and following relationships to test the application
                  functionality.
                </p>
                <button
            ...
            <div className="bg-white/5 rounded-xl p-6 border border-white/10">
              ...
              <div className="space-y-3 text-neutral-300">
                <p>
                  <strong>Users:</strong> 6 test users with different specializations and
                  verification status
                </p>
                <p>
                  <strong>Posts:</strong> 3 sample posts from different sports
                </p>
                <p>
                  <strong>Following:</strong> Pre-configured following relationships between users
                </p>
                <p>
                  <strong>Features:</strong> All users have complete profiles with bios, social
                  media, and specializations
                </p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <p className="text-slate-400 text-sm">
                    Access the complete admin dashboard (requires login)
                  </p>
                </Link>
              </div>
            </div>
