...
export default function DebugPage() {
    ...
    try {
      ...
      if (result.success) {
        setFirebaseTestResult(
          `✅ Firebase connection test successful! Document ID: ${result.docId}`
        );
      } else {
        setFirebaseTestResult(
          `❌ Firebase connection test failed: ${result.error} (Code: ${result.code})`
        );
      }
    } catch (error) {
  ...
  const inspectFirebase = async () => {
    ...
    try {
      ...
      if (result.success) {
        setInspectionResult(
          `✅ Firebase data inspection successful! Found ${result.postsCount} posts and ${result.usersCount} users. Check console for detailed data.`
        );
      } else {
        setInspectionResult(
          `❌ Firebase data inspection failed: ${result.error} (Code: ${result.code})`
        );
      }
    } catch (error) {
  ...
  const createTestTip = async () => {
    ...
    try {
      const testTipData = {
        ...
        content:
          'This is a test tip to verify the posting functionality works correctly. The tip should appear in the center feed immediately after posting.',
        ...
        user: {
          ...
          avatar:
            user.photoURL ||
            'https://images.unsplash.com/photo-1499951360447-b19be8fe80f5?w=96&h=96&fit=crop&crop=face',
        },
      };
